...
if ($dtmfCallEvent->isActive()) {
  ...
  //
   ...
   if ($dtmf == $application->keypadSimulatorExitDigit) {
      ...
      $db->query(sprintf("DELETE FROM %s WHERE call_id = '%s'",  $application->applicationDataTable, $call->id));
   ...
   // if possible
     ...
     if ($level > 0) { 
      ...
      if (array_key_exists((int)$dtmf, get_object_vars($context))) { 
        ...
        addRecord($application->applicationDataTable, array($call->id, $level, $dtmf), array("call_id", "level", "key")); 
      } else {
